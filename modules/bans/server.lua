...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	local accountid, playerip = 'N/A', 'N/A'
	...
	IsBanned(accountid, function(isBanned, banData)
		if isBanned then
	...
	MySQL.Async.execute(
		'INSERT INTO render_bans_history (banid, licenseid, accountid, targetName, sourceName, reason, timeat, expiration, permanent) VALUES (@banid, @licenseid, @accountid, @targetName, @sourceName, @reason, @timeat, @expiration, @permanent)',
		{
			...
			if permanent == 0 then
				...
				SendMessage(source, (('Vous avez banni %s / Durée : Indéterminée / Raison : %s'):format(targetName, reason)))
			end
		end)
end

function DeleteBan(accountid, cb)
	MySQL.Async.execute('DELETE FROM render_bans WHERE accountid = @accountid', {
		['@accountid'] = accountid
	}, function()
		if cb then
			cb()
...
function IsBanned(accountid, cb)
	MySQL.Async.fetchAll('SELECT * FROM render_bans WHERE accountid = @accountid', {
		['@accountid'] = accountid
	}, function(result)
		if #result > 0 then
			cb(true, result[1])
